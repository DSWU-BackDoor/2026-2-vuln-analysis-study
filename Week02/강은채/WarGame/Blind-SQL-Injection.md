...
> 문제: Blind SQL Injection
> 실습자: (이름)
> 실습일: 2026-09-21

> ※ 팀원들이 진행한 command-injection-1(#44), simple_sqli(#24)와 겹치지 않도록
>   기법이 다른 **Blind SQL Injection** 문제로 선정함.

---

## 0. 실습 윤리 / 주의사항
- Dreamhack에서 학습용으로 제공하는 워게임에서만 진행.
- 실제 서비스 대상 공격은 불법. 학습·방어 목적으로만 활용.

---

## 1. 문제 정보
- **문제명**: Blind SQL Injection
...
### (3) ascii() 범위 초과 — 멀티바이트 문자 ★
- `ascii()` 로 뽑으니 일부 글자가 계속 **127(상한)** 로 나옴.
- 원인: DB가 `utf8` 이라 FLAG에 **비ASCII(멀티바이트) 문자**가 포함되어,
  `ascii()` 가 첫 바이트(194~244 등)를 반환 → 127 범위에 걸려 잘림.
- **해결**: `hex(upw)` 로 각 바이트를 16진수(항상 0-9A-F, ASCII)로 추출한 뒤,
  Python에서 `bytes.fromhex(...).decode('utf-8')` 로 복원.

> 교훈: Blind SQLi에서 대상 데이터의 **문자셋/인코딩**을 먼저 고려해야 한다.
>       ASCII 가정은 위험하며, `hex()` 추출이 가장 안전하다.
...
## 11. 배운 점 / 회고
- simple_sqli(인증 우회)와 달리, 데이터를 직접 볼 수 없을 때
  **참/거짓 신호만으로 정보를 복원**하는 기법을 익힘.
- 페이로드 암기보다 **소스코드로 입력값의 흐름과 응답 차이(오라클)를 분석**하는 것이 핵심.
- 자동화 과정에서 **요청 속도, 예외 처리, 대상 인코딩**까지 고려해야 안정적으로 데이터를 뽑을 수 있음을 체감.
